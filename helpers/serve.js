...
const LOG_LEVEL = process.env.SERVE_LOG_LEVEL || 2

const logDebug = (...args) => {
  if (LOG_LEVEL >= 3) {
    console.debug('[serve:debug]', ...args)
  }
}
const logInfo = (...args) => {
  if (LOG_LEVEL >= 2) {
    console.log('[serve:info]', ...args)
  }
}
const logWarn = (...args) => {
  if (LOG_LEVEL >= 1) {
    console.warn('[serve:warn]', ...args)
  }
}
const logError = (...args) => {
  if (LOG_LEVEL >= 0) {
    console.error('[serve:error]', ...args)
  }
}
...
if (!targetDirectory) {
  ...
  logInfo('Usage: bun serve.js <directory-to-serve>')
  logInfo('Example: bun serve.js ./examples')
  process.exit(1)
}
